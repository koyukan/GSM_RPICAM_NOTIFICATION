...
class VideoService {
  ...
  private videoProcesses: Map<string, {
  ...
  public startCapture(config: VideoConfig): VideoStatus {
    ...
    this.checkCommandExists('rpicam-vid').then(exists => {
      if (!exists) {
    ...
    this.videoProcesses.set(id, {
      process: null as any, // Will be set in executeCapture
      status
    });
  ...
  private sanitizeFilename(filename: string): string {
    // Remove any characters that could be problematic for filenames
    return filename
